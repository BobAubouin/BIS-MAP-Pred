...
if os.path.exists(weight_path):
    ...
    # build a model
    cin = Input(batch_shape=(None, 4))
    pin = Input(batch_shape=(None, timepoints, 1))
    rin = Input(batch_shape=(None, timepoints, 1))
    cout = cin
    pout = pin
    rout = rin
    pout = LSTM(lnode, input_shape=(timepoints, 1), activation='tanh')(pout)
    rout = LSTM(lnode, input_shape=(timepoints, 1), activation='tanh')(rout)
    out = Concatenate()([pout, rout, cout]) #, mode='concat'
    out = Dense(fnode, kernel_initializer='he_uniform', activation='relu', activity_regularizer=l2(1.0e-4))(out)
    out = Dropout(0.5)(out)
    out = Dense(1, kernel_initializer='he_uniform', activation='sigmoid')(out)

    model = Model(inputs=[pin, rin, cin], outputs=[out])
    model.compile(loss='mae', optimizer='adam')

    # train the model
    hist = model.fit(train_x, train_y, validation_data=(val_x, val_y), epochs=10, batch_size=batch_size, shuffle=True,
                     callbacks=[ModelCheckpoint(monitor='val_loss', filepath=weight_path, verbose=1, save_best_only=True),
                                EarlyStopping(monitor='val_loss', patience=0, verbose=2, mode='auto')])
