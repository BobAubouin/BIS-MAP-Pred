...
def compute_metrics(data):
    ...
    print(f"${round(MDPE,1)} \pm {round(SD_MDPE,1)}$ & ${round(MDAPE,1)} \pm {round(SD_MDAPE,1)}$ & ${round(RMSE,1)} \pm {round(SD_RMSE,1)}$")
...
dir = "./lee_code"
weight_path = dir + "/weights.hdf5"


test_p, test_r, test_c, test_y = pickle.load(open(dir + cache_path_test, "rb"))
...
def plot_results(y, y_predicted, name, step=1):
    ...
    fig1.line(np.arange(0, len(y)*step, step), y, line_color='navy', legend_label="y")
    fig1.line(np.arange(0, len(y_predicted)*step, step), y_predicted, line_color='red', legend_label="y predicted")
    fig1.xaxis.axis_label = "time [s]"
    fig1.yaxis.axis_label = "y"

    fig2 = figure(width=900, height=450, title="Fitting error")
    fig2.line(np.arange(0, len(y)*step, step), y-y_predicted, line_color='navy', legend_label="y")
    fig2.xaxis.axis_label = "time [s]"
    fig2.yaxis.axis_label = "y"

    y_filt = y
    mytitle = "Fitting test dataset " + name
    fig3 = figure(width=900, height=450, title=mytitle)
...
plot_results(Output_df['true_BIS'], Output_df['pred_BIS'], 'BIS')
