...
@author: aubouinb
"""
"""Script to apply machine learning techniques on vitalDB dataset with the objective to detect anomalous situations"""
...
for y_col in ['BIS', 'MAP']: #
    ...
    try: #Try to load trained regressor
        ...
        if name_rg=='ElasticNet':
            X_train = PolynomialFeatures(degree=2, include_bias=False).fit_transform(Patients_train[X_col].values)   
    ...
    if name_rg=='ElasticNet':
        ...
        X_test = PolynomialFeatures(degree=2, include_bias=False).fit_transform(Patients_test[X_col].values)
