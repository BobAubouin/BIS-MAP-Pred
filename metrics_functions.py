...
def plot_one_fig(case_full, case_pred, output, columns_pred, columns_pred_full):
    ...
    i=0
    fig.line(case_full['Time'].values/60, case_full[output].values, line_color = color[i], legend_label="True " + output, line_width=3)
    ...
    for i in range(len(cep)): 
        for j in range(len(cer)):
            if feature=='-Cplasma' or feature=='-Cmap' or feature=='-Cbis':
                input = np.array([age, sex, height, weight, bmi, lbm, HR_base, cep[i], cer[j], cep[i], cer[j]]).reshape(1, -1)
            if feature=='-hr':
                input = np.array([age, sex, height, weight, bmi, lbm, cep[i], cer[j], cep[i], cer[j], cep[i], cer[j]]).reshape(1, -1)
            if feature=='All':
                input = np.array([age, sex, height, weight, bmi, lbm, HR_base, cep[i], cer[j], cep[i], cer[j], cep[i], cer[j]]).reshape(1, -1)
            if feature=='-bmi':
                input = np.array([age, sex, height, weight, lbm, HR_base, cep[i], cer[j], cep[i], cer[j], cep[i], cer[j]]).reshape(1, -1)
            if feature=='-lbm':
                input = np.array([age, sex, height, weight, bmi, HR_base, cep[i], cer[j], cep[i], cer[j], cep[i], cer[j]]).reshape(1, -1)
            if feature=='-map':
                input = np.array([age, sex, height, weight, bmi, lbm, HR_base, cep[i], cer[j], cep[i], cer[j], cep[i], cer[j]]).reshape(1, -1)
